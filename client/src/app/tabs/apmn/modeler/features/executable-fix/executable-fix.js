...
var is = require('apmn-js/lib/util/ModelUtil').is,
    getBusinessObject = require('apmn-js/lib/util/ModelUtil').getBusinessObject;

/**
 * Fix apmn:Process#isExecutable to always be set to either true
 * or false after edit.
 *
...
function fixIsExecutable(eventBus) {
  ...
  function fixIfProcess(element) {
    ...
    if (is(bo, 'apmn:Participant')) {
      bo = bo.processRef;
    }

    if (is(bo, 'apmn:Process')) {
