...
import CamundaApmnModeler from './modeler';

import { active as isInputActive } from '../../../util/dom/isInput';

import getApmnContextMenu from './getApmnContextMenu';

import { getApmnEditMenu } from './getApmnEditMenu';

import getApmnWindowMenu from './getApmnWindowMenu';

import css from './ApmnEditor.less';
...
export class ApmnEditor extends CachedComponent {
  ...
  async loadTemplates() {
    ...
    const templates = await this.props.onLoadConfig('apmn.elementTemplates');
  ...
  handleChanged = () => {
    ...
    const contextMenu = getApmnContextMenu(newState);

    const editMenu = getApmnEditMenu(newState);

    const windowMenu = getApmnWindowMenu(newState);
  ...
  static createCachedState(props) {
    ...
    const moddleExtensionPlugins = getPlugins('apmn.modeler.moddleExtension');
    ...
    const additionalModules = getPlugins('apmn.modeler.additionalModules') || [];

    const modeler = new CamundaApmnModeler({
      additionalModules,
      exporter: {
        name,
        version
...
export default WithCache(WithCachedState(ApmnEditor));
